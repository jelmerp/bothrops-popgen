...
# NOTE: must be unique per job (not under the shared LOG_DIR) since many
#       loci run concurrently into the same output dir
clipkit_log="$outfile".stdout.txt
set +e
...
# ClipKIT can crash (rather than exit cleanly) when a locus has no sites left
# after trimming - fall back to keeping the untrimmed locus in that case,
# rather than failing the job. The '.untrimmed' marker file lets downstream
# steps identify these loci without a shared/racy tracking file
if [[ "$exit_status" -ne 0 ]]; then
    if grep -q "All sites trimmed from alignment" "$clipkit_log"; then
        log_time "WARNING: all sites were trimmed from $infile - keeping the untrimmed locus instead"
        cp "$infile" "$outfile"
        touch "$outfile".untrimmed
    else
        exit "$exit_status"
    fi
fi
